    ...
    <div className="space-y-8 md:space-y-16">
      ...
      <div className="space-y-4">
        <div className="flex justify-end">
          <Link href="/alumni/register" className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-blue-600 text-white px-8 py-4 rounded-2xl font-black text-[10px] md:text-xs uppercase tracking-[0.2em] hover:bg-blue-700 transition shadow-lg active:scale-95">
      ...
      <div className="grid grid-cols-1 gap-4 lg:hidden">
        {filteredAlumni && filteredAlumni.length > 0 ? (
                ...
                <div className="flex items-center justify-between gap-4 pt-2">
                    {person.social_link ? (
                       <a 
                         ...
                         className="flex-1 flex items-center justify-center gap-2 py-3 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg hover:bg-blue-700 transition-all"
                       >
                         <LinkIcon className="w-3 h-3" />
                         LinkedIn
                       </a>
                    ) : (
                       <div className="flex-1 py-3 bg-slate-100 text-slate-400 rounded-xl text-[10px] font-black uppercase tracking-widest text-center border border-slate-200 italic">
                         Private Profile
                       </div>
                    )}
                </div>
          ...
          <div className="py-20 text-center bg-white rounded-[2rem] border border-slate-100 shadow-lg">
            ...
            <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest">No matching alumni</h3>
          </div>
        )}
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                ...
                <th className="px-10 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Graduation</th>
                <th className="px-10 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Communication</th>
                <th className="px-10 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Professional Info</th>
                <th className="px-10 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] text-center">Portfolio</th>
              </tr>
            </thead>
              ...
              {filteredAlumni && filteredAlumni.length > 0 ? (
                  ...
                  return (
                    <tr key={person.id} className="hover:bg-slate-50 transition-all duration-300 group relative border-b border-slate-50 last:border-0 overflow-hidden">
                      <td className="px-10 py-5 relative z-10">
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 bg-slate-50 text-slate-400 rounded-2xl flex items-center justify-center font-black text-xs border border-slate-100 group-hover:bg-blue-600 group-hover:text-white transition-all duration-300 shrink-0">
                            {initials}
                          </div>
                      ...
                      <td className="px-10 py-5 text-center relative z-10">
                        {person.social_link ? (
                          <a 
                            ...
                            target="_blank" 
                            rel="noopener noreferrer" 
                            className="inline-flex items-center gap-2 px-6 py-3 bg-blue-600 text-white hover:bg-blue-700 rounded-xl transition-all text-[10px] font-black uppercase tracking-widest shadow-lg"
                          >
                            <LinkIcon className="w-3 h-3" />
                            LinkedIn
                          </a>
                        ) : (
                          <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest italic">Private</span>
                        )}
                      </td>
                    </tr>
                  )
                ...
                <tr>
                  <td colSpan={5} className="py-40 text-center">
                    ...
                    <h3 className="text-2xl font-black text-slate-300 uppercase tracking-[0.3em]">No alumni records found</h3>
