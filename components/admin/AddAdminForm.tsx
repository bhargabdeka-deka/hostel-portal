...
export function AddAdminForm({ isSuperAdmin }: { isSuperAdmin: boolean }) {
  ...
  return (
    <section className="bg-white border border-slate-100 p-8 rounded-[2rem] shadow-sm space-y-8">
      <div className="flex items-center gap-4">
        <div className="w-12 h-12 rounded-xl bg-slate-50 flex items-center justify-center text-slate-900 shadow-sm">
            ...
            <div className="relative">
              <Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300" />
                <input 
                  ...
                  className="w-full pl-12 pr-4 py-3.5 bg-slate-50 border border-slate-100 rounded-xl outline-none focus:ring-2 focus:ring-blue-500/10 transition-all font-medium text-slate-700 text-sm" 
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input 
                ...
                required
                placeholder="••••••••" 
                className="w-full pl-12 pr-12 py-3.5 bg-slate-50 border border-slate-100 rounded-xl outline-none focus:ring-2 focus:ring-blue-500/10 transition-all font-medium text-slate-700 text-sm" 
                suppressHydrationWarning
              />
              <button
                type="button"
            ...
            <div className="flex gap-6 items-center">
              <label className="flex items-center gap-2 cursor-pointer group">
                <input 
                  ...
                  value="admin"
                  defaultChecked 
                  className="w-4 h-4 text-blue-600 focus:ring-blue-500/10 border-slate-300 cursor-pointer" 
                />
                <span className="text-sm font-bold text-slate-600 group-hover:text-slate-900 transition-colors">Admin</span>
              </label>
              <label className={`flex items-center gap-2 ${isSuperAdmin ? 'cursor-pointer group' : 'opacity-30 cursor-not-allowed'}`}>
                <input 
                  ...
                  className="w-4 h-4 text-blue-600 focus:ring-blue-500/10 border-slate-300 cursor-pointer" 
                />
                <span className={`text-sm font-bold ${isSuperAdmin ? 'text-slate-600 group-hover:text-slate-900' : 'text-slate-400'}`}>Super Admin</span>
              </label>
            </div>
          </div>
        </div>
