...
export default function RulesPage() {
  ...
  return (
    <div className="py-16 md:py-24 px-6 max-w-5xl mx-auto overflow-x-hidden">
      <div className="text-center mb-16 md:mb-20">
        <div className="inline-flex items-center justify-center gap-2 px-4 py-1.5 bg-slate-100 rounded-full text-[8px] md:text-[10px] font-black uppercase tracking-widest mb-6">
          Code of Conduct
        </div>
      ...
      <div className="grid gap-6 md:gap-8">
        {rules.map((rule, i) => (
          <div key={i} className="group bg-white border border-slate-100 rounded-[2rem] md:rounded-[2.5rem] p-8 md:p-10 flex flex-col sm:flex-row gap-6 md:gap-8 items-start hover:shadow-2xl hover:border-blue-100 transition-all duration-500">
            <div className="w-14 h-14 md:w-16 md:h-16 bg-slate-50 rounded-2xl flex items-center justify-center text-slate-400 group-hover:bg-slate-900 group-hover:text-white transition-all duration-500 shadow-sm shrink-0">
              {rule.icon}
            </div>
            <div className="space-y-3">
              <h3 className="text-xl md:text-2xl font-black text-slate-900 tracking-tight uppercase">{rule.title}</h3>
              <p className="text-sm md:text-base text-slate-500 font-medium leading-relaxed">{rule.details}</p>
            </div>
          </div>
        ))}
      </div>

      <div className="mt-16 md:mt-20 p-8 md:p-12 bg-blue-50/50 border border-blue-100 rounded-[2rem] md:rounded-[2.5rem] flex flex-col md:flex-row items-center gap-8 md:gap-10">
        <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center text-blue-600 shadow-sm shrink-0 border border-blue-100">
