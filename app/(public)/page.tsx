...
export default async function HomePage() {
  ...
  const stats = [
    { label: 'Established', value: '1982', icon: <Calendar className="w-5 h-5" />, color: 'text-rose-600', bgColor: 'bg-rose-50', borderColor: 'border-rose-100' },
    { label: 'Capacity', value: '75', icon: <Users className="w-5 h-5" />, color: 'text-emerald-600', bgColor: 'bg-emerald-50', borderColor: 'border-emerald-100' },
    { label: 'Legacy', value: '40+ Years', icon: <Award className="w-5 h-5" />, color: 'text-amber-600', bgColor: 'bg-amber-50', borderColor: 'border-amber-100' },
  ];
      ...
      <section className="max-w-7xl mx-auto px-6 -mt-12 relative z-20">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {stats.map((stat, i) => (
            <div 
              key={i} 
              className={`p-6 md:p-8 ${stat.bgColor} border ${stat.borderColor} rounded-3xl shadow-xl shadow-slate-200/50 flex items-center gap-6`}
            >
              <div className={`w-12 h-12 rounded-2xl bg-white flex items-center justify-center shrink-0 ${stat.color} shadow-sm`}>
                {stat.icon}
              </div>
              <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 md:gap-12">
          {spirits.map((spirit, i) => (
            ...
            >
              ...
              <div className={`${spirit.color} relative z-10 p-4 bg-white rounded-2xl inline-block shadow-sm`}>{spirit.icon}</div>
              <div className="space-y-4 relative z-10">
                <h3 className={`text-3xl font-bold text-slate-900 tracking-tight font-heading`}>{spirit.title}</h3>
          ...
          {typedNotices.length > 0 ? (
            typedNotices.map((notice: Notice, i: number) => (
              <div key={notice.id} className={`p-8 md:p-12 hover:bg-slate-50 transition-all cursor-default group ${i !== typedNotices.length - 1 ? 'border-b border-slate-100' : ''}`}>
                <div className="text-[10px] font-bold text-slate-400 uppercase mb-4 flex items-center gap-2 tracking-[0.2em]">
