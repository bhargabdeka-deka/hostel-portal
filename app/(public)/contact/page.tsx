  ...
  return (
    <main className="bg-white min-h-screen selection:bg-blue-100 overflow-x-hidden">
      ...
      <section className="max-w-7xl mx-auto px-6 py-12">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-8 max-w-4xl mx-auto">
          {mainContacts.map((c, i) => (
            <div key={i} className="p-8 md:p-10 bg-white border border-slate-100 rounded-[2.5rem] shadow-sm hover:shadow-xl transition-all duration-500 text-center space-y-6 group">
              <div className={`w-14 h-14 md:w-16 md:h-16 rounded-2xl bg-slate-50 flex items-center justify-center mx-auto ${c.color} group-hover:scale-110 transition-transform duration-500 shrink-0`}>
                {c.icon}
              </div>
      ...
      <section className="max-w-7xl mx-auto px-6 py-20">
        <div className="bg-slate-900 rounded-[3rem] md:rounded-[3.5rem] p-8 md:p-12 lg:p-20 relative overflow-hidden text-white shadow-3xl shadow-slate-200">
      ...
      <section className="max-w-7xl mx-auto px-6 py-24">
        <div className="bg-slate-50 rounded-[3rem] md:rounded-[4rem] p-8 md:p-12 lg:p-20 space-y-12 md:space-y-16">
          <div className="text-center">
            <h2 className="text-3xl md:text-5xl font-black text-slate-900 tracking-tight uppercase italic">Current Monitors</h2>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {monitors && monitors.length > 0 ? (
              monitors.map((m) => (
                <div key={m.id} className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl transition-all group flex flex-col gap-3">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center shrink-0">
                      <User className="w-5 h-5" />
                    </div>
                    <div className="min-w-0">
                      <div className="text-[8px] font-black text-blue-600 uppercase tracking-widest truncate">{m.role}</div>
                      <div className="text-sm font-black text-slate-900 tracking-tight truncate uppercase">{m.name}</div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-2 pt-3 border-t border-slate-50">
                    ...
                    {m.phone && (
                      <a 
                        href={`tel:${m.phone}`}
                        className="flex items-center gap-2 text-blue-600 text-[9px] font-black uppercase tracking-widest hover:text-blue-800 transition-colors"
                      >
