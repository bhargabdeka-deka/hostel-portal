  ...
  return (
    <main className="bg-transparent min-h-screen selection:bg-rose-500/20 overflow-x-hidden">
      ...
      <section className="max-w-7xl mx-auto px-6 pt-32 md:pt-40 pb-12 md:pb-16 text-center space-y-6">
        ...
        <h1 className="text-4xl md:text-7xl font-bold text-slate-900 dark:text-white tracking-tight leading-none font-jakarta">Connect.</h1>
        <p className="text-base md:text-xl text-slate-600 dark:text-slate-400 font-medium max-w-2xl mx-auto leading-relaxed px-4">
          ...
          {mainContacts.map((c, i) => {
            ...
            return (
              <Wrapper 
                ...
                className={cn(
                  `p-8 md:p-10 ${c.bgColor} dark:bg-slate-900/40 border ${c.borderColor} dark:border-slate-800 rounded-[2.5rem] shadow-lg dark:shadow-none hover:border-indigo-100 dark:hover:border-indigo-900/50 transition-all duration-300 text-center space-y-6 group block`,
                  c.link && "cursor-pointer active:scale-95 shadow-slate-200/50"
                )}
              >
                <div className={`w-14 h-14 md:w-16 md:h-16 rounded-2xl bg-white dark:bg-slate-800 flex items-center justify-center mx-auto ${c.color} group-hover:scale-105 transition-transform duration-300 shrink-0 shadow-sm dark:shadow-none`}>
                  {c.icon}
                </div>
                <div className="space-y-2">
                  <h3 className="text-[11px] font-bold text-indigo-600 dark:text-indigo-400 tracking-tight font-sans">{c.title}</h3>
                  <p className="text-base md:text-lg font-bold text-slate-900 dark:text-white leading-tight tracking-tight font-sans">{c.detail}</p>
                </div>
              </Wrapper>
            );
          })}
      ...
      <section className="max-w-7xl mx-auto px-6 py-20">
        <div className="bg-slate-50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 rounded-[3rem] md:rounded-[3.5rem] p-8 md:p-12 lg:p-20 relative overflow-hidden shadow-xl dark:shadow-none">
          <div className="relative z-10 flex flex-col lg:flex-row items-center gap-12">
            <div className="flex-1 space-y-6 text-center lg:text-left">
              <div className="inline-flex items-center px-4 py-1.5 rounded-full bg-indigo-100 dark:bg-indigo-950/30 border border-indigo-200 dark:border-indigo-900/50 text-indigo-600 dark:text-indigo-400 text-[11px] font-bold tracking-tight font-sans">
                Administrative leadership
              </div>
              <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-slate-900 dark:text-white tracking-tight leading-tight font-jakarta">Office of the <br/>Superintendent</h2>
              <p className="text-slate-600 dark:text-slate-400 font-medium leading-relaxed max-w-md mx-auto lg:mx-0 text-sm md:text-base">
            ...
            <div className="w-full lg:w-auto">
              <div className="bg-indigo-50 dark:bg-slate-900/60 border border-indigo-100 dark:border-slate-800 rounded-[2rem] md:rounded-[2.5rem] p-8 md:p-12 min-w-0 md:min-w-[400px] shadow-lg dark:shadow-none shadow-indigo-200/20">
                <div className="text-[11px] font-bold text-indigo-600 dark:text-indigo-400 tracking-tight mb-4 font-sans">Current superintendent</div>
                <div className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-white mb-8 tracking-tight font-sans">Mr. Jiten Borgohain</div>
                <a 
      ...
      <section className="max-w-7xl mx-auto px-6 py-24">
        <div className="bg-white dark:bg-slate-900/40 border border-slate-100 dark:border-slate-800 rounded-[3rem] md:rounded-[4rem] p-8 md:p-12 lg:p-20 space-y-12 md:space-y-16 shadow-xl dark:shadow-none">
          <div className="text-center">
            <h2 className="text-3xl md:text-5xl font-bold text-slate-900 dark:text-white tracking-tighter font-jakarta">Current monitors</h2>
          </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="text-xl font-bold text-slate-900 dark:text-white tracking-tight font-sans group-hover:text-indigo-600 transition-colors leading-tight">{m.name}</div>
                    </div>
